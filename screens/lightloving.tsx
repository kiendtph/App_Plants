...
const lightLoving = ({navigation}:any) => {
  const { themeStyles } = useTheme();
  const plants = useSelector(state => state.listTodoStore.plants || []);
  const lightLovingPlants = plants.filter(plant => plant.features === 'Ưa sáng');
  return (
    <View style={[styles.container,{backgroundColor:themeStyles.background}]}>
      <Text style={[styles.title,,{color:themeStyles.text}]}>Sản phẩm ưa sáng</Text>
      <FlatList
        data={lightLovingPlants}
